...
백엔드는 전체 결제 건수·금액, `SETTLED` 기준 누적 정산 건수·금액, 결제 상태별 건수·금액을 집계 API로 제공합니다. 프론트 대시보드의 결제·정산 지표는 이 서버 집계를 사용하며, 주문 수와 최근 결제·실패 목록은 목록 조회 API가 없어 현재 브라우저 세션을 기준으로 표시합니다.
...
백엔드 테스트는 H2 기반으로 결제 승인, 잔액 부족 실패, 취소 환불, 정산 후 취소 불가, 상태 전이 규칙, 전체 결제·정산 집계를 검증합니다.

동일 Idempotency Key 경합 테스트는 같은 주문과 같은 키로 요청 `5,000건`을 만들고, 최대 `200개` 동시성으로 실행합니다. 이 테스트는 승인 성공 `1건`, 중복 요청 `4,999건`, 결제 생성 `1건`, 잔액 `1회` 차감만 발생하는지 확인합니다.

## 프론트엔드 화면

- 대시보드: 서버 전체 결제 수, 승인 완료, 취소, 정산 완료, 실패 건수와 세션 기준 주문·최근 내역
- 주문 생성
...
브라우저 세션에 아직 확인한 주문/결제가 없는 상태입니다. 결제·정산 지표는 서버 전체 집계 API를 기준으로 표시하고, 주문 수와 최근 내역은 현재 브라우저 세션을 기준으로 표시합니다.
...
- 대시보드는 서버 집계 API를 사용하지만, 주문 목록과 결제 목록 조회 API는 아직 없어 최근 내역은 현재 브라우저 세션 기준으로 표시합니다.
