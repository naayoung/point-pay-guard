...
테스트는 H2 기반으로 결제 승인, 잔액 부족 실패, 취소 환불, 정산 후 취소 불가, 상태 전이 규칙, 동일 멱등성 키 경합, 전체 결제·정산 집계를 검증합니다.

동일 멱등성 키 경합 통합 테스트는 같은 주문과 같은 Idempotency Key로 요청 `5,000건`을 만들고, 최대 `200개` 동시성으로 실행합니다. 최초 요청이 멱등성 키를 선점한 상태에서 나머지 요청이 처리 중인 키와 경합하도록 제어하고, 최종적으로 승인 성공 `1건`, 중복 요청 `4,999건`, 결제 생성 `1건`, 잔액 `1회` 차감만 발생하는지 확인합니다.
